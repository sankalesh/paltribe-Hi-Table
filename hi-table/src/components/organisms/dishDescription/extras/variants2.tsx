...
//     console.log(" cjw cwjw wcwcw", newQuantity, item, itemId);
...
//   };
  ...
  const getQuantity = (itemId: string) => {
    ...
    return (
      cartItem?.[dishData?.id]?.variants?.[index]?.extra?.[itemId]?.quantity ||
      0
    );
  };
  ...
  // console.log("bwic wc wc  wsji nbwc",dishData.extras?.[state?.activeExtra]?.items)
    ...
    <>
      ...
      <div className="px-2">
        <Section
          title={"Extras"}
          tag="h5"
          tagClassName="font-semibold text-base-content !text-xl"
        >
      ...
      <div className="mx-6 my-2 overflow-x-auto border rounded-lg border-[#2C62F0]">
        <div className="px-3 py-3 prose">
          ...
          {(dishData.extras?.[state?.activeExtra]?.compulsory || 0) > 0 && (
            <span className="text-xs font-medium text-base-content/60">
              Select at least{" "}
              {dishData.extras?.[state?.activeExtra]?.compulsory || 0} of the
              type of {dishData.extras?.[state?.activeExtra]?.name || ""}.
            </span>
          )}
        </div>
        ...
        <table className="table w-full table-compact table-zebra">
          <tbody>
            {(dishData.extras?.[state?.activeExtra]?.items || []).map(
              (item, index: number) => {
                ...
                return (
                  <tr className="flex flex-row" key={itemId}>
